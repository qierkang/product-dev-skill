...
1. 若当前运行端可识别 `ui-ux-pro-max`，优先用它补设计基线
2. 若不可识别，自动回退到 `shared/references/design/`
3. 不允许因为外部设计 skill 缺失而跳过 UI 文档阶段
4. 回退时默认不得改用 `frontend-design` 或其他通用设计 skill 充当兜底
